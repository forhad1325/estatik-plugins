jQuery(function($) {
    ...
    function renderPins() {
        ...
        $('table.widefat tbody tr').each(function() {
            ...
            if (!isNaN(x) && !isNaN(y)) {
                ...
                const iconFile = statusIcons[status] || 'default.svg';
                ...
                $pin.css({ left: x + '%', top: y + '%' }).append('<img src="' + iconPath + '" alt="' + status + '">');
    ...
    $(document).on('mouseenter', '.plat-map-pin', function(e) {
        ...
        $('body').append($tooltip);
        $tooltip.css({ position: 'absolute', top: e.pageY + 10, left: e.pageX + 10, display: 'none' }).fadeIn(200);
    });

    $(document).on('mousemove', '.plat-map-pin', function(e) {
        $('.plat-map-tooltip').css({ top: e.pageY + 10, left: e.pageX + 10 });
    });
