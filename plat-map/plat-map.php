 ...
 * Plugin URI:        https://github.com/forhad1325/estatik-plugins/
 * Description:       Creates interactive property plot maps for the Estatik Real Estate plugin.
 * Version:           1.0.0
...
class Plat_Map_Plugin {
    ...
    public function enqueue_frontend_assets() {
        wp_enqueue_style('plat-map-style', plugin_dir_url(__FILE__) . 'assets/css/style.css', [], filemtime(plugin_dir_path(__FILE__) . 'assets/css/style.css'));
        wp_enqueue_script('plat-map-script', plugin_dir_url(__FILE__) . 'assets/js/main.js', ['jquery'], filemtime(plugin_dir_path(__FILE__) . 'assets/js/main.js'), true);
    }
    ...
    public function render_plat_map_shortcode($atts) {
        ...
        if (!$post_id) return '';
        $image_id = get_post_meta($post_id, '_plat_map_image_id', true);
